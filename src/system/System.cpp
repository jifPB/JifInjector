...
HMODULE Jif::System::GetModuleByKeyword(HANDLE hProcess, const std::wstring &keyword)
{
	DWORD lpcbNeeded;
	HMODULE hMods[1024];
	std::wstring modName;
...
DWORD Jif::System::GetProcessIdByName(const std::wstring &processName)
{
	DWORD processId = 0;
...
HANDLE Jif::System::GetProcessHandleById(DWORD processId)
{
...
HANDLE Jif::System::GetProcessHandleByName(const std::string &processName)
{
...
LPVOID Jif::System::AllocateMemoryInProcess(HANDLE hProcess, SIZE_T size)
{
...
BOOL Jif::System::FreeMemoryInProcess(HANDLE hProcess, LPVOID lpAddress, SIZE_T size)
{
	return VirtualFreeEx(hProcess, lpAddress, size, MEM_RELEASE);
}

void Jif::System::RunThreadInProcess(
...
std::vector<DWORD> Jif::System::GetRunningProcessesId()
{
...
std::string Jif::System::GetProcessNameByID(DWORD processId)
{
...
std::vector<std::string> Jif::System::GetRunningProcessesNames()
{
	std::vector<std::string> processesNames;
	std::vector<DWORD> processesID = GetRunningProcessesId();
